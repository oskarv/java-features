...
class RunnablePattern {
    ...
    public static void main (String [] argv) {
        ...
        // Does the some as lines above
        new Thread(() -> System.out.println("What does this do? -- " + Thread.currentThread().getName())).start();

        // Executor doesn't create new thread, it always reuses the same one
        ...
        // Runnable can't return results or throw errors, Callable is used when we need those two
        Callable<List<String>> callable = () -> Arrays.asList("**", "****", "*", "***");
        Future<List<String>> futureResult = executor.submit(callable);

        try {
            // get is a blocking call until results are available!
            // we can pass time to get() method so we don't block the world forever
